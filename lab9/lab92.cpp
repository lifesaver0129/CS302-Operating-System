...
int find_start_point(int *arr) {
    int target = abs(start_point - arr[0]);
    int target_index = 0;
    for (int i = 1; i < track_length; i++) {
        if (target < abs(start_point - arr[i]))
            continue;
        target = arr[i];
        target_index = i;
    }
...
void SCAN(int *track_queue) {
    ...
    if (start_point < track_queue[track_length - 1]) {
        if (not_seen_first) {
            range += abs(start_point);
            cout << start_point << connect << 0;
            range += abs(track_queue[list_start_point]);
            cout << connect << track_queue[list_start_point];
            if (track_length > 1) {
                range += abs(track_queue[list_start_point + 1] - track_queue[list_start_point]);
                cout << connect << track_queue[list_start_point + 1];
            }
        } else {
            range += abs(track_queue[0]);
            cout << connect << 0;
            range += abs(track_queue[list_start_point + 1]);
            cout << connect << track_queue[list_start_point + 1];
        }
...
void CSCAN(int *track_queue) {
    ...
    if (start_point < track_queue[track_length - 1]) {
        ...
        range += abs(start_point);
        cout << connect << 0 << connect << max_track;
